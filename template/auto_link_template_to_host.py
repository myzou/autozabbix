...
#日志文件
log=log.LOG("auto_link_template_to_host")

#配置文件
list_file="../list/auto_link_template_to_host.txt"
...
with open(list_file,"r+") as f:
    ...
    for line in f.readlines():
        ...
        if zabbix.host_exists(host_ip):
            ...
            #主机未加入模板
            if host_id_list not in monitoered_host:
                monitoered_host.append(host_id_list)
                zabbix.template_to_host(template_id,monitoered_host)
                log.info("主机%s成功加入监控模板%s" % (host_ip,template_name))
            #主机已在模板
            else:
                log.info("主机%s已存在模板%s" % (host_ip,template_name))
        #主机不存在
        else:
